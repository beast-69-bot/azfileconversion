...
def _plan_kb() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="?? Custom Amount", callback_data="pay:custom"),
...
async def pay_plan_callback(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if plan == "custom":
        ...
        await callback.message.reply(
            format_msg("?? Custom Amount", sections=[("", f"Enter your desired amount (minimum INR {MIN_CUSTOM_PAY_INR:.0f}):")]),
            parse_mode="HTML",
        )
        return
...
async def pay_custom_amount_handler(message: Message, state: FSMContext) -> None:
    try:
        ...
        await message.reply(format_msg("?? Invalid", sections=[("", "Please enter a valid number, e.g. 75")]), parse_mode="HTML")
        return
    if amount < MIN_CUSTOM_PAY_INR:
        await message.reply(format_msg("?? Too Low", sections=[("Minimum", f"INR {MIN_CUSTOM_PAY_INR:.0f}")]), parse_mode="HTML")
        return
...
async def pay_utr_text_handler(message: Message, state: FSMContext) -> None:
    ...
    if not utr:
        await message.reply(format_msg("?? Empty", sections=[("", "Please type a valid UTR number.")]), parse_mode="HTML")
        return
...
async def pay_screenshot_wrong_type(message: Message, state: FSMContext) -> None:
    await message.reply(format_msg("?? Photo Required", sections=[("", "Please send a photo (screenshot), not a file or text.")]), parse_mode="HTML")
...
async def approve_cmd(message: Message) -> None:
    ...
    if len(parts) < 2:
        await message.reply(format_msg("?? Usage", sections=[("", code("/approve <request_id>"))]), parse_mode="HTML")
        return
...
async def reject_cmd(message: Message) -> None:
    ...
    if len(parts) < 2:
        await message.reply(format_msg("?? Usage", sections=[("", code("/reject <request_id> [reason]"))]), parse_mode="HTML")
        return
    req_id = parts[1].strip()
...
async def private_media_handler(message: Message, state: FSMContext) -> None:
    ...
    if not media:
        ...
        if message.chat.type == "private" and is_admin(message.from_user.id if message.from_user else None):
            await message.reply(format_msg("?? Unsupported", sections=[("", "Send a document, video, audio, animation, voice, video_note, or photo.")]), parse_mode="HTML")
        return
    ...
    if not section_id:
        if message.chat.type == "private":
            await message.reply(format_msg("?? No Active Section", sections=[("", f"Set one first: {code('/addsection <name>')}")]), parse_mode="HTML")
        return
    ...
    if message.chat.type == "private":
        await message.reply(
            format_msg("? File Uploaded", sections=[
                ...
                ("Normal", link("?? Open Stream", build_link(normal_token))),
                ("Premium", link("?? Download Stream", build_link(premium_token))),
